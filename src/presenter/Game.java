...
public class Game extends Canvas implements Runnable
{
    ...
    // Collision handler
    public boolean isColl(GameObject player, GameObject food){
        if(player.getX()+player.getWidth() >= food.getX()){  //left
            if(player.getX() <= food.getX()+food.getWidth()){  //right
                if(player.getY()+player.getHeight() >= food.getY()){  //top
                    if(player.getY() <= food.getY()+food.getHeight()){  //bottom
                        return true;
                    }
                }
            }
    ...
    public void loop()
    {
        ...
        {   
            ...
            for(int i = 0; i < handler.count(); i++)
            {
                ...
                }else if(handler.get(i).getType().equals("Food")){
                    food = handler.get(i);
                }
            }
            if(player!=null && food!=null){
                if(this.isColl(player,food)){
                    this.addScore(10);
                    handler.remove(food);
                    handler.addFood();
                }
